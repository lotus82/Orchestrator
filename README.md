...
Итог: **«ИИ» здесь — **дискриминативная модель компьютерного зрения**, которая формирует **Ground Truth** для честного скоринга. Это хорошо масштабируется: тот же каркас можно подключить к **детекции других классов**, **трекерам**, или в будущем — к **мультимодальным моделям** (например, аудио + видео для событий).
